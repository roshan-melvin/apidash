...
final webSocketMessagesProvider = Provider.autoDispose
    .family<List<WebSocketMessage>, String>((ref, id) {
...
final webSocketEventLogProvider = Provider.autoDispose
    .family<List<WebSocketEvent>, String>((ref, id) {
      final liveStateAsync = ref.watch(webSocketStateProvider);
...
final webSocketStateSyncProvider = Provider<void>((ref) {
  ...
  ref.listen(webSocketStateProvider, (_, next) {
    if (next.hasValue && selectedId != null) {
      ...
      if (model != null && model.apiType == APIType.websocket) {
        final savedMessages = next.value!.messages
            .map(
              (m) => WebSocketSavedMessage(
        ...
        final updatedWs =
            (model.websocketRequestModel ?? kWebSocketRequestEmptyModel)
                .copyWith(
