...
class GrpcReflectionService {
  final Map<String, Map<String, $descriptor.FileDescriptorProto>>
  _descriptorCache = {};
  final Map<String, (String, String)> _methodTypeCache = {};

  Future<Map<String, $descriptor.FileDescriptorProto>>
  loadDescriptorsViaReflection({
  ...
  Future<List<String>> _listServices(
    ...
    try {
      await for (final resp in responseStream.timeout(
        const Duration(seconds: 15),
        onTimeout: (sink) {
          sink.addError(
            GrpcReflectionException(
              'Server Reflection timed out. Check TLS/SSL settings and port.',
            ),
          );
        },
      )) {
    ...
    try {
      await for (final resp in responseStream.timeout(
        ...
        if (resp.hasFileDescriptorResponse()) {
          for (final fdBytes
              in resp.fileDescriptorResponse.fileDescriptorProto) {
            ...
            for (final dep in fd.dependency) {
              if (!descriptorMap.containsKey(dep)) {
                await _resolveDescriptorsForFile(
                  client,
                  dep,
                  descriptorMap,
                  host,
  ...
  GrpcMethodSignature extractMethodSignature({
    ...
    if (_methodTypeCache.containsKey(cacheKey)) {
      ...
      if (serviceDesc == null)
        throw GrpcReflectionException('Service not found: $serviceName');
      ...
      if (methodDesc == null)
        throw GrpcReflectionException(
          'Method ${methodName} not found in ${serviceName}',
        );
    ...
    if (inputMessageDesc == null)
      throw GrpcReflectionException('Input message type not found: $inputType');

    final requestFields = inputMessageDesc.field
        .map((field) {
    ...
    if (outputMessageDesc == null)
      throw GrpcReflectionException(
        'Output message type not found: $outputType',
      );

    final responseFields = outputMessageDesc.field
        .map((field) {
          final kind = _kindForField(field);
          return GrpcRequestFieldSchema(
            name: field.name,
  ...
  $descriptor.DescriptorProto? findMessageDescriptor(
    String typeName,
...
class _DynamicMessage {
  ...
  void mergeFromJson(
    ...
    for (final field in descriptor.field) {
      ...
      fields[field.number] = _coerceJsonValue(
        json[jsonKey],
        field,
        descriptors,
      );
  ...
  void mergeFromBuffer(
    ...
    while (offset < buffer.length) {
      ...
      final field = descriptor.field.firstWhere(
        (f) => f.number == fieldNum,
        orElse: () => $descriptor.FieldDescriptorProto()..number = -1,
      );
      if (field.number == -1) {
  ...
  dynamic _coerceJsonValue(
    ...
    switch (field.type.value) {
      ...
      case 12:
        return jsonValue is List
            ? Uint8List.fromList(
                jsonValue.whereType<num>().map((e) => e.toInt()).toList(),
              )
            : utf8.encode(jsonValue.toString());
      default:
        return jsonValue;
    }
  }
  ...
  dynamic _valueToJson(
    ...
    if (field.label.value == 3 && value is List)
      return value.map((v) => _valueToJson(v, field, descriptors)).toList();
  ...
  static (dynamic, int) _readFieldValue(
    ...
    if (wireType == 1)
      return (Uint8List.sublistView(buffer, offset, offset + 8), offset + 8);
    if (wireType == 5)
      return (Uint8List.sublistView(buffer, offset, offset + 4), offset + 4);
    if (wireType == 2) {
