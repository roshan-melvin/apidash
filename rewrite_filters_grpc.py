...
def update_grpc(content):
    ...
    # Fix event logic
    grpc_event_filter_old = """    final filteredEvents = _filterEventString.isEmpty
        ? events
        : events
              .where(
    ...
    grpc_event_filter_new = """    var typeFilteredEvents = events;
    if (_filterEventIndex == 1) {
      typeFilteredEvents = events.where((e) => e.type == GrpcEventType.error || e.type == GrpcEventType.disconnect).toList();
    ...
    final filteredEvents = _filterEventString.isEmpty
        ...
        : typeFilteredEvents
              ...
              .toList();"""
    content = content.replace(grpc_event_filter_old, grpc_event_filter_new)

    # 1. Replace the Message Filter Row (first tab)
    # The first tab row has TextField and SegmentedButton
    ...
    # 2. Replace Event Tab Row
    # Current Event tab only has a TextField. Wrap in a Row and add ADDropdownButton
...
process_file('lib/widgets/grpc_response_pane.dart', update_grpc)
