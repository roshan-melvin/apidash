...
def process_file(filename, success_cond):
    ...
    final filteredEvents = events.where((e) {{
      ...
      if (_filterEventString.isNotEmpty) {{
        final query = _filterEventString.toLowerCase();
...
"""
    ...
    content = re.sub(
        r"    final filteredEvents = _filterEventString\.isEmpty.*?\.toList\(\);",
...
"""
    ...
    content = re.sub(
        r"              return Padding\([\s\S]*?onChanged: \(v\) => setState\(\(\) => _filterEventString = v\),\n                \),\n              \);",
        new_event_textfield.strip(),
        content
    )

    # Update topics white inside (Messages filter too)
    content = re.sub(
