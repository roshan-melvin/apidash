...
1. Have you worked on or contributed to a FOSS project before? Can you attach repo links or relevant PRs?
   - Yes. My active Proof-of-Concept PR #1613 (which is an updated version of the initial PR #1529) for APIDash demonstrates a fully working MCP & CLI integration inside the existing monorepo. The implementation spans three packages: `apidash_mcp` (Dart MCP server), `apidash_mcp_core` (shared Dart library), and `apidash_cli` (headless terminal executor).
...
import { executeHttpRequest } from "apidash_mcp_core";
...
| File | What It Powers |
|---|---|
| `executor.dart` | `http-send-request` tool + `apidash-cli request` command |
| `graphql.dart` | `graphql-execute-query` tool + `apidash-cli graphql` command |
| `ai.dart` | `ai-llm-request` tool + `apidash-cli ai` command |
| `codegen.dart` | `generate-code-snippet` tool + `apidash-cli codegen` command |
| `workspace.dart` | All `save-*` / `update-*` tools + `apidash-cli save` / `set` commands |
