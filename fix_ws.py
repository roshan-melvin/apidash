...
message_bubble_pattern = r"class _MessageBubble extends StatelessWidget \{.*?return Container\(.*?decoration: BoxDecoration\(.*?\).*?padding: kP8,.*?child: Column\(.*?crossAxisAlignment: CrossAxisAlignment\.start,.*?children: \[.*?Row\(.*?children: \[(.*?)\]\).*?kVSpacer8.*?Text\((.*?)\).*?\]\).*?\);\n  \}\n\}"
