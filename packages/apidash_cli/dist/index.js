 ...
 */
import { getMcpWorkspaceData, getSyncFilePath, updateMcpWorkspaceData, executeHttpRequest, executeGraphQLRequest, executeAIRequest, AI_PROVIDERS, generateCode, SUPPORTED_GENERATORS } from "@apidash/mcp-core";
import fs from "fs";
...
function statusBadge(code) {
    if (code === 0)
        return `${c.bgRed}${c.white} ERR ${c.reset}`;
    if (code < 300)
        return `${c.bgGreen}${c.white} ${code} ${c.reset}`;
    if (code < 400)
        return `${c.bgYellow}${c.white} ${code} ${c.reset}`;
...
function hr(char = "─", width = 60) { return dim(char.repeat(width)); }
// ─────────────────────────────────────────────────────────────
// Argument helpers
// ─────────────────────────────────────────────────────────────
...
function collectRepeated(args, flag) {
    const out = [];
    for (let i = 0; i < args.length; i++) {
...
// ─────────────────────────────────────────────────────────────
function printBody(body, maxLines = 50, truncateAt = 4000) {
...
function cmdList() {
    ...
    if (requests.length === 0) {
        console.log(yellow("  No requests found."));
        return;
    }
...
async function cmdRun(input, timeoutMs) {
    const { requests } = getMcpWorkspaceData();
    let req;
    ...
    if (!isNaN(idx) && idx > 0 && idx <= requests.length)
        ...
        req = requests.find(r => r.id === input);
    if (!req) {
...
function dispatchHttpResult(result, method, url) {
    if (!result.success) {
    ...
    if (ph.length > 0) {
        ...
        for (const [k, v] of ph)
            console.log(`    ${cyan(k.padEnd(24))} ${dim(String(v))}`);
        console.log();
    }
...
function cmdEnv(scope) {
    ...
    for (const env of envsToPrint) {
        ...
        for (const v of env.values) {
            if (!v.enabled && v.enabled !== undefined)
                continue;
...
function cmdSet(scope, key, value, secret = false) {
    ...
    if (!saved) {
        console.error(red("  ✗ Failed to save workspace file."));
        process.exit(1);
    }
    console.log(dim(`  Workspace: ${getSyncFilePath()}\n`));
}
function cmdCodegen(input, lang) {
    const { requests } = getMcpWorkspaceData();
    let req;
    const idx = parseInt(input);
    if (!isNaN(idx) && idx > 0 && idx <= requests.length)
    ...
    if (!SUPPORTED_GENERATORS.includes(lang)) {
        console.error(red(`\n  ✗ Generator "${lang}" unsupported.\n`));
        process.exit(1);
    }
    const code = generateCode(lang, { method: req.method, url: req.url, headers: req.headers, body: req.body });
...
function cmdInfo() {
    ...
    if (lastUpdated)
        console.log(`  ${dim("Last sync:  ")} ${dim(new Date(lastUpdated).toLocaleString())}`);
...
// ─────────────────────────────────────────────────────────────
async function cmdRequest(rawArgs) {
    ...
    if (Object.keys(headers).length) {
        for (const [k, v] of Object.entries(headers))
            console.log(`  ${dim("›")} ${cyan(k)}: ${dim(v)}`);
    }
    if (body)
        console.log(`  ${dim("body:")} ${dim(body.slice(0, 80))}${body.length > 80 ? "…" : ""}`);
    console.log(hr());
    const result = await executeHttpRequest({ method, url, headers, body, timeoutMs });
    dispatchHttpResult(result, method, url);
    // Optional: codegen
    if (codegenLang && SUPPORTED_GENERATORS.includes(codegenLang)) {
        const code = generateCode(codegenLang, { method, url, headers, body });
...
// ─────────────────────────────────────────────────────────────
async function cmdGraphQL(rawArgs) {
    const url = rawArgs[0];
    if (!url) {
        console.error(red("\n  ✗ Missing URL."));
    ...
    if (operation)
        console.log(`  ${dim("operation:")} ${magenta(operation)}`);
    if (Object.keys(variables).length)
        console.log(`  ${dim("variables:")} ${dim(JSON.stringify(variables))}`);
    ...
    if (errors && Array.isArray(errors) && errors.length > 0) {
        console.log(`\n  ${red("⚠ GraphQL Errors:")}`);
        errors.forEach((e, i) => console.log(`    ${dim(`${i + 1}.`)} ${red(e?.message ?? JSON.stringify(e))}`));
    }
    console.log(`\n${bold("  GraphQL Data:")}`);
    console.log(hr("─", 56));
...
async function cmdAI(rawArgs) {
    ...
    if (system)
        console.log(`  ${dim("system:")} ${dim(system.slice(0, 80))}${system.length > 80 ? "…" : ""}`);
    ...
    console.log(`\n  ${statusBadge(status)}  ${dim(`${duration}ms`)}${totalTokens ? `  ${dim(`tokens: ${inputTokens ?? "?"}→${outputTokens ?? "?"} = ${totalTokens}`)}` : ""}${finishReason ? `  ${dim(`stop: ${finishReason}`)}` : ""}\n`);
...
// ─────────────────────────────────────────────────────────────
function saveRequestToWorkspace(req) {
    const workspace = getMcpWorkspaceData();
...
async function main() {
    ...
    switch (command) {
        ...
        // ── Ad-hoc commands ──────────────────────────────────────
        case "request":
            await cmdRequest(args.slice(1));
            break;
        case "graphql":
            await cmdGraphQL(args.slice(1));
            break;
        case "ai":
