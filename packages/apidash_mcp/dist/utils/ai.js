...
export async function executeAIRequest(ctx) {
    const { url, apiKey, model, messages, systemPrompt, temperature = 0.7, maxTokens = 1024, headers = {}, timeoutMs = 60_000, } = ctx;
    ...
    try {
        ...
        const raw = typeof response.data === "object"
            ? JSON.stringify(response.data, null, 2)
            : String(response.data ?? "");
        const d = typeof response.data === "object" ? response.data : {};
        const choice = d?.choices?.[0];
        const content = choice?.message?.content ?? d?.content?.[0]?.text ?? d?.message?.content ?? "";
        const usage = d?.usage;
        return {
